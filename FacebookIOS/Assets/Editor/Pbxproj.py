...
class Pbxproj(object):
	...
	def __init__(self, name, xcode_version = None):
		...
		self.logger = logger = logging.getLogger('pbxproj')
		logFileHandler = logging.FileHandler('pbxproj.log')
	...
	def get_project_data(self):
		if self._project_data is None:
			if not os.path.exists(self.path()):
				self.logger.info("Couldn't find the project at this path:")
	...
	def dependencies(self):
		...
		if project_data is None:
			self.logger.error("Unable to open the project file at this path (is it readable?): "+self.path())
			return None
		...
		if not result:
			self.logger.error("Can't recover: unable to find the project version for your target at: "+self.path())
			return None
		...
		if self.configurationListGuid:
			...
			if not match:
				self.logger.error("Couldn't find the configuration list.")
				return False
		...
		if not result:
			self.logger.error("Can't recover: Unable to find the build phases from your target at: "+self.path())
			return None
		...
		if not match:
			self.logger.error("Couldn't find the Frameworks phase from: "+self.path())
			self.logger.error("Please add a New Link Binary With Libraries Build Phase to your target")
			self.logger.error("Right click your target in the project, Add, New Build Phase,")
			self.logger.error("  \"New Link Binary With Libraries Build Phase\"")
			return None
		...
		if not result:
			self.logger.error("Unable to get dependencies from: "+self.path())
			return None
		...
		if not result:
			self.logger.error("Unable to get product guid from: "+self.path())
			return None
	...
	def add_buildfile(self, name, file_ref_hash, default_guid):
		...
		if not match:
			self.logger.error("Couldn't find PBXBuildFile section.")
			return None
		...
		if match:
			...
			self.logger.info("This build file already exists: "+buildfile_hash)
	...
	def add_filereference(self, name, file_type, default_guid, rel_path, source_tree):
		...
		if match:
			self.logger.info("This file has already been added.")
			...
			if not match:
				self.logger.error("Couldn't find the PBXFileReference section.")
				return False
	...
	def add_file_to_group(self, name, guid, group):
		...
		if not match:
			self.logger.error("Couldn't find the "+group+" children.")
			return False

		(children,) = match.groups()
		match = re.search(re.escape(guid), children)
		if match:
			self.logger.info("This file is already a member of the "+name+" group.")
		else:
			match = re.search('\/\* '+re.escape(group)+' \*\/ = \{\n[ \t]+isa = PBXGroup;\n[ \t]+children = \(\n', project_data)

			if not match:
				self.logger.error("Couldn't find the "+group+" group.")
				return False
	...
	def add_file_to_phase(self, name, guid, phase_guid, phase):
		...
		if not match:
			self.logger.error("Couldn't find the "+phase+" phase.")
			return False
		...
		if match:
			self.logger.info("The file has already been added.")
		else:
			match = re.search(re.escape(phase_guid)+" \/\* "+phase+" \*\/ = {(?:.|\n)+?files = \(\n", project_data)
			if not match:
				self.logger.error("Couldn't find the "+phase+" files")
				return False
	...
	def add_file_to_resources_phase(self, name, guid):
		if self._resources_guid is None:
			self.logger.error("No resources build phase found in the destination project")
			self.logger.error("Please add a New Copy Bundle Resources Build Phase to your target")
			self.logger.error("Right click your target in the project, Add, New Build Phase,")
			self.logger.error("  \"New Copy Bundle Resources Build Phase\"")
			return False
	...
	def get_filerefguid_from_buildfileguid(self, buildfileguid):
		...
		if not match:
			self.logger.error("Couldn't find PBXBuildFile row.")
			return None
	...
	def get_filepath_from_filerefguid(self, filerefguid):
		...
		if not match:
			self.logger.error("Couldn't find PBXFileReference row.")
			return None
	...
	def get_built_sources(self):
		...
		if not match:
			self.logger.error("Couldn't find PBXSourcesBuildPhase section.")
			return None
	...
	def get_built_headers(self):
		...
		if not match:
			self.logger.error("Couldn't find PBXHeadersBuildPhase section.")
			return None
	...
	def add_dependency(self, dep):
		...
		self.logger.info("")
		self.logger.info("Project path:    "+project_path)
		self.logger.info("Dependency path: "+dep_path)
		self.logger.info("Relative path:   "+rel_path)
		
		tthash_base = self.get_hash_base(dep.uniqueid())
	
		###############################################
		self.logger.info("")
		self.logger.info("Step 1: Add file reference to the dependency...")
		
		self.set_project_data(project_data)
		pbxfileref_hash = self.add_filereference(dep._project_name+'.xcodeproj', 'pb-project', tthash_base+'0', rel_path, 'SOURCE_ROOT')
		project_data = self.get_project_data()

		self.logger.info("Done: Added file reference: "+pbxfileref_hash)
		
		###############################################
		self.logger.info("")
		self.logger.info("Step 2: Add file to Frameworks group...")
		...
		self.logger.info("Done: Added file to Frameworks group.")
		
		###############################################
		self.logger.info("")
		self.logger.info("Step 3: Add dependencies...")
		...
		if not match:
			self.logger.info("\tAdding a PBXTargetDependency section...")
			match = re.search('\/\* End PBXSourcesBuildPhase section \*\/\n', project_data)
			
			if not match:
				self.logger.error("Couldn't find the PBXSourcesBuildPhase section.")
				return False
			...
			if match:
				...
				self.logger.info("This dependency already exists.")
		...
		self.logger.info("Done: Added dependency.")


		###############################################
		self.logger.info("")
		self.logger.info("Step 3.1: Add container proxy for dependencies...")

		containerExists = False

		match = re.search('\/\* Begin PBXContainerItemProxy section \*\/\n((?:.|\n)+?)\/\* End PBXContainerItemProxy section \*\/', project_data)
		if not match:
			self.logger.info("\tAdding a PBXContainerItemProxy section...")
			match = re.search('\/\* End PBXBuildFile section \*\/\n', project_data)
			
			if not match:
				self.logger.error("Couldn't find the PBXBuildFile section.")
				return False
			...
			if match:
				self.logger.info("This container proxy already exists.")
		...
		self.logger.info("Done: Added container proxy.")


		###############################################
		self.logger.info("")
		self.logger.info("Step 3.2: Add module to the dependency list...")
		...
		if not match:
			self.logger.error("Couldn't find the dependency list.")
			return False
		else:
			(dependencylist, ) = match.groups()
			match = re.search(re.escape(pbxtargetdependency_hash), dependencylist)
			if match:
				self.logger.info("This dependency has already been added.")
		...
		self.logger.info("Done: Added module to the dependency list.")


		###############################################
		self.logger.info("")
		self.logger.info("Step 4: Create project references...")

		match = re.search('\/\* Begin PBXProject section \*\/\n((?:.|\n)+?)\/\* End PBXProject section \*\/', project_data)
		if not match:
			self.logger.error("Couldn't find the project section.")
			return False
		...
		if not match:
			self.logger.info("Creating project references...")
			match = re.search('projectDirPath = ".*?";\n', project_section)
			if not match:
				self.logger.error("Couldn't find project references anchor.")
				return False
			...
			if match:
				...
				self.logger.info("This product group already exists: "+productgroup_hash)
		...
		if not reference_exists:
			...
			if not match:
				self.logger.error("Missing the project references item.")
				return False
		...
		self.logger.info("Done: Created project reference.")

		###############################################
		self.logger.info("")
		self.logger.info("Step 4.1: Create product group...")

		match = re.search('\/\* Begin PBXGroup section \*\/\n', project_data)
		if not match:
			self.logger.error("Couldn't find the group section.")
			return False
		...
		if match:
			self.logger.info("This product group already exists.")
			(children, ) = match.groups()
			match = re.search('([A-Z0-9]+) \/\* '+re.escape(dep._product_name)+' \*\/', children)
			if not match:
				self.logger.error("No product found")
				return False
		...
		self.logger.info("Done: Created product group: "+lib_hash)



		###############################################
		self.logger.info("")
		self.logger.info("Step 4.2: Add container proxy for target product...")
		...
		self.logger.info("Done: Added target container proxy.")


		###############################################
		self.logger.info("")
		self.logger.info("Step 4.3: Create reference proxy...")

		referenceExists = False

		match = re.search('\/\* Begin PBXReferenceProxy section \*\/\n((?:.|\n)+?)\/\* End PBXReferenceProxy section \*\/', project_data)
		if not match:
			self.logger.info("\tAdding a PBXReferenceProxy section...")
			match = re.search('\/\* End PBXProject section \*\/\n', project_data)
			
			if not match:
				self.logger.error("Couldn't find the PBXProject section.")
				return False
			...
			if match:
				self.logger.info("This reference proxy already exists.")
		...
		self.logger.info("Done: Created reference proxy.")


		###############################################
		self.logger.info("")
		self.logger.info("Step 5: Add target file...")

		self.set_project_data(project_data)
		libfile_hash = self.add_buildfile(dep._product_name, lib_hash, tthash_base+'5')
		project_data = self.get_project_data()

		self.logger.info("Done: Added target file.")
		

		###############################################
		self.logger.info("")
		self.logger.info("Step 6: Add frameworks...")

		self.set_project_data(project_data)
		self.add_file_to_frameworks_phase(dep._product_name, libfile_hash)
		project_data = self.get_project_data()

		self.logger.info("Done: Adding module.")
